...
export async function startController(){
 ...
 await new Promise(resolve=>navigator.locks.request('gridiron-controller:'+base.pathname,{ifAvailable:true},async lock=>{
  ...
  worker=new Worker(new URL('runtime-worker.js?v=crew111',base));
 ...
 channel.onmessage=async({data})=>{
  ...
  if(data.kind==='command'){
   try{const {commands}=await import('./commands.js?v=crew111');const command=commands[data.command];if(!command)throw Error('Unknown button');
